...
Uma vez que Alice e Bob compartilham uma chave secreta, eles podem utilizá-la para criptografar mensagens com segurança. No entanto, garantir que essa chave não foi interceptada é uma tarefa não trivial em sistemas clássicos.
...
O protocolo pode ser descrito em etapas:

### 1. Preparação

Alice gera duas sequências aleatórias:

* uma sequência de bits
* uma sequência de bases

Ela então prepara qubits de acordo com essas escolhas e os envia para Bob.

### 2. Medição

Bob, sem saber as bases usadas por Alice, escolhe aleatoriamente uma base para medir cada qubit.

### 3. Comparação de bases

Após a transmissão, Alice e Bob comunicam, por um canal clássico, quais bases foram utilizadas — **sem revelar os valores dos bits**.

Eles mantêm apenas os bits nos quais utilizaram a mesma base.

### 4. Detecção de interceptação

Alice e Bob comparam uma parte dos bits restantes para estimar a taxa de erro.

* Se a taxa for baixa → a chave é considerada segura
* Se for alta → houve interferência (possível interceptação)

### 5. Geração da chave

Os bits restantes formam a chave secreta compartilhada.
...
A presença de um interceptador pode ser modelada como um canal quântico adicional atuando sobre o estado transmitido:
...
Essa composição reduz a fidelidade do estado transmitido e introduz ruído observável por Alice e Bob.
...
Esse comportamento pode ser entendido como um aumento da entropia do estado recebido por Bob, refletindo a degradação da informação original.
...
Além disso, ela é uma das primeiras aplicações práticas da informação quântica, com implementações experimentais já realizadas.
