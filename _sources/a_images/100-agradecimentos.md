# Autores e Agradecimentos

O *Aprenda Ket* é desenvolvido pelo Grupo de Computação Quântica da Universidade Federal de Santa Catarina. A plataforma surgiu como uma extensão do projeto *Ket* — plataforma brasileira de programação quântica — adaptando o trabalho de conclusão de curso do professor Giovani Goraiebe Pollachini para servir como apoio à disciplina FSC7152 – Computação Quântica, ministrada pelo professor Eduardo Duzzioni.

Atualmente, o site continua sendo desenvolvido no GCQ com o propósito de tornar a computação quântica mais acessível, didática e inclusiva, contribuindo para a democratização de uma área ainda marcada por grandes barreiras de entrada. Para alcançar esse objetivo, contamos com a colaboração de diversas pessoas envolvidas tanto na estruturação do projeto quanto na produção e disponibilização de materiais complementares que enriquecem o ambiente de aprendizagem.

Desde já, registramos nosso sincero agradecimento a todos que contribuem para o crescimento e fortalecimento do *Aprenda Ket*.

## Autores

O *Aprenda Ket* nasceu com o objetivo de tornar a computação quântica mais acessível, didática e intuitiva. Este projeto só foi possível graças ao apoio, incentivo e conhecimento compartilhado por pessoas incríveis ao longo do caminho.

<div style="display:flex; align-items:center; gap:15px; margin-bottom:20px;">
  <img src="https://raw.githubusercontent.com/evandro-crr/ket-book/main/book/a_images/evandro.png"   
  style="
    width:100px;
    height:100px;
    object-fit:cover;
    border-radius:10%;
    flex-shrink:0;
  ">
  <div>
    <b>Evandro Chagas Ribeiro da Rosa</b><br>
    Evandro é cofundador e CTO da Quantuloop, startup de computação quântica. Doutorando e mestre em Ciência da Computação pela UFSC, sua pesquisa foca na construção de compiladores para computação quântica. Idealizador da plataforma de programação quântica Ket, lidera seu desenvolvimento no Grupo de Computação Quântica da UFSC.
  </div>
</div>


<div style="display:flex; align-items:center; gap:15px; margin-bottom:20px;">
  <img 
    src="https://raw.githubusercontent.com/evandro-crr/ket-book/main/book/a_images/Duzzioni.jpeg" 
    style="
    width:100px;
    height:100px;
    object-fit:cover;
    border-radius:10%;
    flex-shrink:0;
  "
  >
  
  <div>
    <b>Eduardo Inacio Duzzioni</b><br>
    Eduardo é membro fundador do Grupo de Computação Quântica da UFSC, tem experiência na área de Física Quântica, atuando principalmente nos seguintes temas: computação quântica e informação quântica. Atualmente é professor titular no Departamento de Física da Universidade Federal de Santa Catarina, onde atuou como sub-coordenador do Programa de Pós-graduação em Física no período de 2015-2021. É co-fundador da startup QuanBy Computação Quântica.
  </div>
</div>

<div style="display:flex; align-items:center; gap:15px; margin-bottom:20px;">
  <img src="https://raw.githubusercontent.com/evandro-crr/ket-book/main/book/a_images/jerusa.jpeg" 
  style="
    width:100px;
    height:100px;
    object-fit:cover;
    border-radius:10%;
    flex-shrink:0;
  "
>
  <div>
    <b>Jerusa Marchi</b><br>
    Jerusa é Professora Associada no Departamento de Informática e Estatística da Universidade Federal de Santa Catarina. Tem experiência na área de Ciência da Computação, com ênfase em Matemática da Computação, atuando principalmente nos seguintes temas: inteligência artificial (lógica e representação do conhecimento, processos de mudança de crenças, sistemas cognitivos), teoria da computação (reduções e satisfação booleana) e computação quântica.
  </div>
</div>

<div style="display:flex; align-items:center; gap:15px;">
  <img src="https://raw.githubusercontent.com/evandro-crr/ket-book/main/book/a_images/miguel.jpg" style="
    width:100px;
    height:100px;
    object-fit:cover;
    border-radius:10%;
    flex-shrink:0;
  "
>
  <div>
    <b>Miguel Estivalet Pinto</b><br>
    Miguel é estudante de Ciências da Computação na UFSC e membro do Grupo de Computação Quântica da UFSC. Seus interesses envolvem computação quântica, com ênfase em: Simulação Quântica, Aprendizado de Máquina Quântica e Divulgação Científica. 
  </div>
</div>

## Colaboradores

Na ampliação do Aprenda Ket, contamos com o apoio de diversos profissionais que contribuíram com a disponibilização de materiais para a plataforma. Agradecemos pela confiança, colaboração e dedicação de todos que ajudaram a fortalecer este projeto e a expandir o acesso ao conhecimento em computação quântica.

<div style="display:flex; align-items:center; gap:15px;">
  <img src="https://raw.githubusercontent.com/evandro-crr/ket-book/main/book/a_images/giovani.jpg" style="
    width:100px;
    height:100px;
    object-fit:cover;
    border-radius:10%;
    flex-shrink:0;
  "
>
  <div>
    <b>Giovani Goraiebe Pollachini</b><br>
    Giovani é formado em Física, Matemática e Engenharia Eletrônica, possui experiência como professor substituto na UFSC e na UDESC (Joinville/SC), lecionando disciplinas de Matemática e Física para Engenharias. Seus interesses são: Computação Quântica, Ciência de Dados e demais áreas correlatas.
  </div>
</div>

<div style="display:flex; align-items:center; gap:15px;">
  <img src="https://raw.githubusercontent.com/evandro-crr/ket-book/main/book/a_images/guilherme_temporao.jpg" style="
    width:100px;
    height:100px;
    object-fit:cover;
    border-radius:10%;
    flex-shrink:0;
  "
>
  <div>
    <b>Guilherme Temporão</b><br>
    Guilherme é Professor Associado da PUC-Rio e leciona disciplinas de graduação e pós-graduação, além de atuar como pesquisador na área de comunicação quântica, metrologia quântica e computação quântica. É pesquisador principal do projeto Rede Rio Quântica e coordenador do NITeQ/PUC-Rio (Núcleo Interdisciplinar de Tecnologias Quânticas). Suas demais áreas de interesse incluem redes quânticas, instrumentação optoeletrônica e educação em Engenharia.
  </div>
</div>

<div style="display:flex; align-items:center; gap:15px;">
  <img src="https://raw.githubusercontent.com/evandro-crr/ket-book/main/book/a_images/let.jpg" style="
    width:100px;
    height:100px;
    object-fit:cover;
    border-radius:10%;
    flex-shrink:0;
  "
>
  <div>
    <b>Leticia Bertuzzi</b><br>
  Leticia é mestra e graduada em Física Bacharelado pela Universidade Federal de Santa Catarina (UFSC). Tem experiência na área da Física, com ênfase em Computação Quântica e Física Quântica, também tem experiência em Física da Matéria Condensada e Mecânica Estatística com foco em Nanomateriais. Atualmente, está cursando o doutorado em Física na UFSC. Participa no grupo de pesquisa: Grupo de Computação Quântica da Universidade Federal de Santa Catarina (UFSC) - GCQ e do O Grupo de Informação Quântica do Sul(UFSC)-GIQSul .
  </div>
</div>
